...
class HikesView {
  renderHikeList(hikeListElement, hikeList, comments) {
    ...
    hikeList.forEach(hike => {
      ...
      hikeListElement.appendChild(this.renderOneHikeLight(hike, comments));
    });
  }
  ...
  renderOneHikeLight(hike, comments) {
    ...
    item.innerHTML += this.renderComments(hike.name, comments);
  ...
  renderComments(name, comments) {
    ...
    comments.filter(comment => comment.name === name).forEach(comment => {
      item += `<p>${comment.comment}</p>`;
    });
  ...
  renderAddComment(name) {
    let item = 
    `<form>
      <input id='hikeName' type='hidden' value='${name}' />
  ...
  renderOneHikeFull(parent, hike, comments) {
    ...
    item.innerHTML += this.renderComments(hike.name, comments);
    item.innerHTML += this.renderAddComment(hike.name);
